...
namespace ZGWUI
{
    public partial class Form1 : Form
    {
        ...
        Int32 count_message;
        byte direction_ask_conso;

        public struct device_announce
        {
           public UInt16 u16ShortAddr;
           public byte u8SrcEndPoint;
           public  UInt16 type;
           public  UInt16 power;
        }

        device_announce sensor_smart_plug_xiaomi;
        ...
        private void GUIinitialize()
        {
            ...
            label5.Text = "count message";
            label6.Text = "tension de la pile";

            comboBox_direction.Items.Add("TO SERVER");
            comboBox_direction.Items.Add("TO CLIENT");
        ...
        private void aboutToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                MessageBox.Show("Temperature xiaomi - Version " + version);
            }
        ...
        *    }
        ...
        // trnsmission des commandes de la cluster librairie
        ...
        *    transmitCommand(0x0130, u8Len, commandData);
        ...
        // affiche en clair les messages sous la forme se string du buffer de reception
        ...
        // Received message parser ( routine thread du message)
            ...
            break;
                */
                ...
                case 0x804E:
                    {
                        ...
                        if (u8NbTableListCount > 0)
                        {
                            ...
                            for (i = 0; i < u8NbTableListCount; i++)
                            {
                                ...
                                richTextBoxMessageView.Text += "\n";
                                // update info
                                search_xiaomi_smart_plug(u8DeviceType, u16NwkAddr);
                            }
                            for (i = 0; i < u8NbTableListCount; i++)
                            {
                ...
                case 0x8100: // Read attribute response
                    {
                        ...
                        display_conso_xiaomi_smart_plug(au8Data, u16ClusterId);
                    }
                    break;
                ...
                case 0x8102:
                    {
                        ...
                        UInt16 u16AttribId = 0;
                        UInt16 u16AttributeSize = 0;
                        // adresse du device (visible dans discover
                        u16SrcAddr = au8Data[1];
                        u16SrcAddr <<= 8;
                        u16SrcAddr |= au8Data[2];
                        // adresse cluster
                        ...
                        // type à 0 dans xiami
                        displayAttribute(u16AttribId, au8Data[9], au8Data, 12, u16AttributeSize);
                        display_attribute_xiaomi_temp(au8Data, u16ClusterId, u16Length);
                        display_attribute_xiaomi_smart_plug(au8Data, u16ClusterId, u16Length);
                    }
                    break;
                ...
                case 0x004D:
                    ...
                    // on recupere les infos du capteurs suite a son annonce et notemment
                    // 
                    // sensor_announce = get_sensor_announce(Uint16 u16ShortAddr,byte[] au8AttribData)
        ...
        // affiche dans la fenetre raw data les donnees reçus (buffer de 1024 octets)
        ...
        // affichage dans la fenetre RAW DATA
        ...
        // ***********************************
        // proprietes des champs a remplir
        // ***********************************
        ...
        // on recupere les composantes du capteurs (short adress, type etc ...)
        private void get_sensor_announce()
        {
        ...
        // decodage temperature cluster 402,405 + tension pile


        // affiche en clair les messages sous la forme se string du buffer de reception
        #region message temperature humidité functions

        //  private void displaytemperature();
        private void display_attribute_xiaomi_temp(byte[] au8AttribData,UInt16 u16ClusterId, UInt16 u16AttributeSize)
        {
            UInt16 temperature;
            UInt16 humidity;
            double val;
            ...
            time_now = DateTime.Now.Hour.ToString("D2");
            time_now += ":";
            time_now += DateTime.Now.Minute.ToString("D2");
            time_now += ":";
            time_now += DateTime.Now.Second.ToString("D2");
            time_now += ".";
            time_now += DateTime.Now.Millisecond.ToString("D3");         


            // test si c'est une trame propriétaire
            if (u16AttributeSize == 48)
            {
                tension_pile = au8AttribData[14];
                tension_pile <<= 8;
                tension_pile |= au8AttribData[13];
                val = Convert.ToDouble(tension_pile);
                val = val / 1000.0;
                tension.Text = val.ToString("f3");
                label6.Text = "tension de la pile";
            }       
            ...
            switch (u16ClusterId)
            {
                case 0x0402:
                    ...
                    temperature_xiaomi.Text = val.ToString("f2");
                ...
                case 0x0405:
                    ...
                    humidity_xiaomi.Text = val.ToString("f2");
                    count_message = count_message + 1;
                    break;
                default:
                    temperature = 0;
            ...
            time_receive = time_now;

            label5.Text = "count message";

            
            count_mess_xiaomi.Text = count_message.ToString();
        ...
        private void textBoxPermitJoinAddr_TextChanged(object sender, EventArgs e)
        {

        }

        #region, smart plug xiaomi

        // bouton smart
        // envoie commande 
        private void power_socket_Click(object sender, EventArgs e)
        {
            ...
            if  (power_socket.Text == "OFF")
                {
                    power_socket.Text = "ON";
                // on envoie la trame
                // on récupere les parametres 
                // recuperation de la Short address
                u8AddrMode = 0x02;
                u16ShortAddr = sensor_smart_plug_xiaomi.u16ShortAddr;
                u8SrcEndPoint = sensor_smart_plug_xiaomi.u8SrcEndPoint;
                u8DstEndPoint = sensor_smart_plug_xiaomi.u8SrcEndPoint;
                u8CommandID = 0x01;
                send_command_on_off(u8AddrMode, u16ShortAddr, u8SrcEndPoint, u8DstEndPoint, u8CommandID);
            }
            else if (power_socket.Text == "ON")
                {
                    power_socket.Text = "OFF";
    ...
    // envoie donnee conso
    private void sendReadAttrib_conso_smart_plug(UInt16 u16ShortAddr, byte u8SrcEndPoint, byte u8DstEndPoint, UInt16 u16ClusterID, byte u8Direction, byte u8ManuSpecific, UInt16 u16ManuID, byte u8AttribCount, UInt16 u16AttribID1)
        {
            byte[] commandData = null;
            commandData = new byte[14];
            byte u8Len = 0;
        ...
        // envoi de la commande ON/OFF pour la prise
        // normalement le smrt/plug repon d'abord
        // 1.Réponse de la prise (Statut OK) :
        // 2. Retour de l’état de la prise :
        // 3. Retour d’information de la prise pilotée(inconnu pour le moment) :
        private void send_command_on_off(byte u8AddrMode, UInt16 u16ShortAddr, byte u8SrcEndPoint, byte u8DstEndPoint, byte u8CommandID)
        {
            byte[] commandData = null;
            commandData = new byte[6];
            ...
            // source end point = dest endpoint
            // Build command payload   
            commandData[0] = u8AddrMode;
            commandData[1] = (byte)(u16ShortAddr >> 8);
            commandData[2] = (byte)u16ShortAddr;
        ...
        //  affichage statut du smart plug 
        private void display_attribute_xiaomi_smart_plug(byte[] au8AttribData, UInt16 u16ClusterId, UInt16 u16AttributeSize)
        {
            Byte status;
            Byte state_plug;
            UInt16 u16SrcAddr;
            ...
            switch (u16ClusterId)
            {
                case 0x0000: // General : Basic)
                             // status du 
                    
                    u16SrcAddr = au8AttribData[1];
                    u16SrcAddr <<= 8;
                    u16SrcAddr |= au8AttribData[2];
                    sensor_smart_plug_xiaomi.u16ShortAddr = u16SrcAddr;

                    sensor_smart_plug_xiaomi.u8SrcEndPoint = au8AttribData[3];

                 
                    break;
                case 0x0006: // General : ON/OFF)
                             // status de l'etat du smart plug
                    u16SrcAddr = au8AttribData[1];
                    u16SrcAddr <<= 8;
                    u16SrcAddr |= au8AttribData[2];
                    sensor_smart_plug_xiaomi.u16ShortAddr = u16SrcAddr;
                    sensor_smart_plug_xiaomi.u8SrcEndPoint = au8AttribData[3];
                    state_plug = (au8AttribData[11]);
                    if (state_plug == 1)
                    {
                        state.Checked = true;
                    }
                    else if (state_plug == 0)
                    {
                        state.Checked = false;
                    }
                    ...
                    case 0x000C: // conso electrique
                    if (power_socket.Text.Equals("ON"))
                    {
                        conso_smart = au8AttribData[11];
                        conso_smart <<= 8;
                        conso_smart |= au8AttribData[12];
                        tmp = (double)(conso_smart - 16519) / 9.96;
                        conso.Text = tmp.ToString("F2");
                    }
                    else
                    {
                        conso.Text = "";
        ...
        // affichage de la consommation sur read attributes
        // sur requete read_attributes (0x8100) 
        // formule deduit empiriquement conso = val - 16519 / 9,96
        // (mesure 7w,15689 et 60 w, 17117))
        private void display_conso_xiaomi_smart_plug(byte[] au8AttribData, UInt16 u16ClusterId)
        {
            Byte status;
            Byte state_plug;
            UInt16 u16SrcAddr;
            ...
            if (u16ClusterId == 0x000c)
            {
                conso_smart = au8AttribData[12];
                conso_smart <<= 8;
                conso_smart |= au8AttribData[13];
                tmp = (double)(conso_smart - 16519)/ 9.96;              
                conso.Text = tmp.ToString("F2");
            }
            else
        ...
        // search le router apres une commande LQI
        private void search_xiaomi_smart_plug(byte u8DeviceType,UInt16 u16SrcAddr)
        {
           

            if (u8DeviceType == 1) // router
            { 
                sensor_smart_plug_xiaomi.u16ShortAddr = u16SrcAddr;
                sensor_smart_plug_xiaomi.u8SrcEndPoint = 0x01;
            }
            else if (u8DeviceType == 2) // end device
            {
               //
            }
            ...
            {
                sensor_smart_plug_xiaomi.u16ShortAddr = 0x0000;
            }

        }

        // bouton ask conso
        private void conso_smart_Click(object sender, EventArgs e)
        {
            ...
            u8DstEndPoint = 0x02;
            u16ClusterID = 0x000C;
            sendReadAttrib_conso_smart_plug(u16ShortAddr,u8SrcEndPoint,u8DstEndPoint,u16ClusterID, (byte)comboBox_direction.SelectedIndex, 0,0,1,0x0055);
            // u16ShortAddr, u8SrcEndPoint, u8DstEndPoint, u16ClusterID, u8Direction, u8ManuSpecific, u16ManuID, u8AttribCount, u16AttribID1

        }
        //  selecyion server/client
        ...
        private void comboBoxReadAttribDirection_SelectedIndexChanged(object sender, EventArgs e)
        {

        }


        // IHM , bouton lampe hue
        //
        ...
        private void lamp_osram_Click(object sender, EventArgs e)
        {
